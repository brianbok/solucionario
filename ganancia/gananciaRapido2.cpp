...
int ganancia( int P, vector< PQ > fabricantes, vector< PQ > compradores, int & Fab, int & Comp )
{
	...
	for (const auto& p : ps) {
		if (!p.esFabricante) {
			if (p.cantidad * minPrecio.first <= P) {
				auto ganancia = p.cantidad * (p.precio - minPrecio.first);
				if (res < ganancia) {
					res = ganancia;
	...
	for (const auto& p : ps) {
		if (!p.esFabricante) {
			auto ganancia = p.cantidad * p.precio - minFabrica.first;
			if (ganancia > res) {
				res = ganancia;
			...
			tint costo = p.precio * p.cantidad;
			if (costo <= P) {
				if (minFabrica.first > costo) {
